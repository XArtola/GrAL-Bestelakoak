...
function copyActiveFileToConsole() {
    ...
    // Method 2: Try to get content directly from the editor's DOM
    const getContentFromDOM = (editor) => {
        try {
            log('Attempting to extract content from editor DOM...');
            
            // Try to find text content in various editor elements
            const contentElements = [
                // Try different potential content containers
                ...Array.from(editor.querySelectorAll('.view-lines .view-line')),
                ...Array.from(editor.querySelectorAll('.mtk')),
                editor.querySelector('.monaco-editor-background')
            ].filter(Boolean);
            
            if (contentElements.length === 0) {
                log('❌ No content elements found in editor');
                return null;
            }
            
            // If we found view-lines, combine their text content
            if (contentElements[0].classList.contains('view-line')) {
                const lines = contentElements.map(line => line.textContent);
                return lines.join('\n');
            }
            
            // Otherwise, return the text content of the first element
    ...
    // Method 3: Try to access Monaco editor API directly - SIGNIFICANTLY ENHANCED
    ...
    const executeExtraction = async () => {
        ...
        // Try new direct access method first (most likely to get complete content)
        ...
        if (!content) {
            getContentViaVSCodeAPI(); // This is more of a notification than expecting immediate content
        }

        // Step 3: Output the results
        if (content) {
            log('✅ Successfully extracted file content:');
