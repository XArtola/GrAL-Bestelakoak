...
function getVSCodeEditorContent() {
    try {
        // New method: Progressive scrolling extraction to ensure we get all content
        const progressiveScrollExtraction = () => {
            console.log("🔍 Forcing editor to render complete file content...");
            
            // Find the editor element
            const editorElement = document.querySelector('.monaco-editor.focused') || 
                                document.querySelector('.monaco-editor');
            if (!editorElement) {
                console.log("❌ No editor element found for content forcing");
                return null;
            }
            ...
            if (!linesContainer) {
                console.log("❌ No lines container found");
                return null;
            }
            
            console.log("✅ Found editor components needed for extraction");
            ...
            console.log(`📐 Editor dimensions - ScrollHeight: ${scrollHeight}, ViewportHeight: ${clientHeight}`);
            
            // Set to gather lines
            const processedLineIds = new Set();
            const allLines = [];
            
            // Function to extract current visible lines
            const extractVisibleLines = () => {
                const lineElements = Array.from(linesContainer.querySelectorAll('.view-line'));
                console.log(`👁️ Currently visible lines: ${lineElements.length}`);
                ...
                lineElements.forEach(line => {
                    // Create a unique ID for the line to avoid duplicates
                    const lineId = line.style.top + '|' + line.textContent;
                    if (!processedLineIds.has(lineId)) {
                        processedLineIds.add(lineId);
                        newLines.push(line.textContent);
                    }
                });
                
                // Add new lines to our collection
                if (newLines.length > 0) {
                    console.log(`➕ Found ${newLines.length} new lines (total: ${allLines.length + newLines.length})`);
                    allLines.push(...newLines);
                    return true;
                }
                console.log("ℹ️ No new content found in this view");
                return false;
            };
            
            // Aggressive force rendering approach - three phases:
            
            // Phase 1: Jump directly to start, middle, and end of document
            return new Promise(async (resolve) => {
                console.log("🔄 PHASE 1: Jump scanning to force rendering...");
                
                // Start at the top
                console.log("⬆️ Jumping to document start");
                scrollable.scrollTop = 0;
                await delay(300);
                extractVisibleLines();
                
                // Jump to middle
                console.log("↕️ Jumping to document middle");
                scrollable.scrollTop = Math.floor(scrollHeight / 2);
                await delay(300);
                extractVisibleLines();
                
                // Jump to bottom
                console.log("⬇️ Jumping to document end");
                scrollable.scrollTop = scrollHeight;
                await delay(300);
                extractVisibleLines();
                
                // Phase 2: Use executeEdits if possible to ensure full file loading
                console.log("🔄 PHASE 2: Attempting to trigger full document loading...");
                
                try {
                    // Access the internal editor instance if possible
                    const editorInstances = document.querySelectorAll('.monaco-editor');
                    for (const editorEl of editorInstances) {
                        // Try triggering a Select All command which forces loading
                        if (editorEl._modelData?.instance?.trigger) {
                            console.log("🔍 Triggering Select All to force complete document loading");
                            editorEl._modelData.instance.trigger('contentForcing', 'editor.action.selectAll', {});
                            await delay(200);
                            // Restore cursor
                            editorEl._modelData.instance.trigger('contentForcing', 'cursorMove', {});
                        }
                        
                        // Try directly forcing visible range update
                        if (editorEl._modelData?.view?.viewLines?._updateVisibleLines) {
                            console.log("🔍 Directly calling internal update method");
                            editorEl._modelData.view.viewLines._updateVisibleLines(true);
                            await delay(200);
                        }
                    }
                } catch (e) {
                    console.log("⚠️ Editor API direct access attempt failed:", e);
                }
                
                // Phase 3: Progressive scrolling through document
                console.log("🔄 PHASE 3: Progressive scrolling through document...");
                
                // Smaller step size and more delay for better rendering
                ...
                // Progressive scroll from top to bottom
                while (currentPosition < scrollHeight) {
                    // Update progress indicator
                    if (progressCounter % 10 === 0) {
                        const percentage = Math.round((currentPosition / scrollHeight) * 100);
                        const marker = progressMarkers[Math.floor(progressCounter / 10) % 2];
                        console.log(`${marker} Scrolling: ${percentage}% (position ${currentPosition}/${scrollHeight})`);
                    }
                ...
                console.log("↩️ Restoring original scroll position");
                scrollable.scrollTop = originalScrollTop;
                
                // Combine all lines
                const completeContent = allLines.join('\n');
                console.log(`✅ Extraction complete! Retrieved ${allLines.length} total lines.`);
                
                // Copy to clipboard and log
                copyToClipboard(completeContent);
                logFormattedContent(completeContent);
                
                resolve(completeContent);
            });
        };

        // Try model-based extraction methods first (they get complete content)
        ...
        console.log("API-based methods failed to retrieve content, trying progressive scroll extraction...");
        return progressiveScrollExtraction().then(content => {
            if (content) {
                return content;
            } else {
...
// Instead of immediately calling both functions, execute them sequentially
(async function() {
    console.log("Starting enhanced editor content extraction...");
    debugEditorObjects();
    await delay(500); // Give time for debug info to be logged
    const content = await getVSCodeEditorContent();
