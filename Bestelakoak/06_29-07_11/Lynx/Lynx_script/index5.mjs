...
async function getInfoFromURL(baseurl, findurl, depth, obj, debug = false) {
    if (depth === 0) {
        ...
        try {
           ...
           // console.log(`Salida del comando 1: ${stdoutAux}`);
            ...
            if (stdoutAux.includes('400. Se trata de un error.')) {
                ...
                const parsedInfo = infoObj.stdoutPart2.map((line) => {
                    ...
                    if (text.startsWith('http://') || text.startsWith('https://')) {
                        ...
                        //const [type, name] = line.split(',');
                        //const [, , name2] = name.split(' ');
                        return { number, type: 'Button' };
                    }
                    else if (text.startsWith('Text area')) {

                        if (text.includes(',')) {
                            const [type, name] = line.split(',');
                            const [, , name2] = name.split(' ');
                            return { number, type: 'Text area', name: name2 };
                        }
                        else {
                            return { number, type: 'Text area' };
                        }
                    }

                    else if (text.startsWith('Radio button, ')) {
                        const [type, name, value] = line.split(',');
                        const [, , name2] = name.split(' ');
                        const [, , value2] = value.split(' ');
                        return { number, type: 'Radio button', name: name2, value: value2 };
                    }
...
// radio button const url = 'https://codepen.io/LukyVj/pen/bGxwWVv';
const url = 'https://designsystem.digital.gov/components/checkbox/';
const depth = 1;
const obj = {};
...
getInfoFromURL(url, url, depth, obj, false)
    .then((result) => {
        //console.log(result);
        fs.writeFileSync("dataRadioButton.json", JSON.stringify(result));
    })
    .catch((error) => {
        console.error(error);
    });
