...
describe("Transaction Feed", function () {
  ...
  describe("renders and paginates all transaction feeds", function () {
    it("renders transactions item variations in feed", function() {}).as("mockedPublicTransactions");
  ...
  describe("filters transaction feeds by amount range", function () {
    ...
    _.each(feedViews, (feed, feedName) => {
      it(`filters ${feedName} transaction feed by amount range`, function() {}).should("have.class", "Mui-selected");
  ...
  describe("Feed Item Visibility", () => {
    it("mine feed only shows personal transactions", function() {}).then((contacts: Contact[]) => {
    ...
    it("first five items belong to contacts in public feed", function() {}).then((contacts: Contact[]) => {
        ctx.contactIds = contacts.map((contact) => contact.contactUserId);
      });
    ...
    it("friends feed only shows contact transactions", function() {}).then((contacts: Contact[]) => {
        ctx.contactIds = contacts.map((contact) => contact.contactUserId);
      });
