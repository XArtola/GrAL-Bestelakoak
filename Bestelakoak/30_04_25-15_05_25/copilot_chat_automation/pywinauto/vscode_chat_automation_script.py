...
def get_prompt_files():
    ...
    try:
        ...
        print(f"Se encontraron {len(files)} archivos de prompt (sin duplicados):")
...
def automate_vscode():
    ...
    try:
        ...
        for idx, current_prompt_file in enumerate(prompt_files):
            ...
            try:
                ...
                try:
                    ...
                    with open(error_file_path, "w", encoding="utf-8") as f:
                        ...
                        f.write(contenido)
                    print(f"✅ Contenido guardado en archivo alternativo: {error_file_path}")
                    archivo_salida = error_file_path  # Actualizar la ruta para el registro de timestamps
                except Exception as e2:
                    print(f"❌ Error crítico al guardar, incluso en archivo alternativo: {e2}")
            ...
            print("Enfocando el chat (Ctrl+Shift+I)...")
            send_keys_safely(main_window, "^+i")
            time.sleep(1)

            print("Abriendo el panel de chat (Ctrl+Alt+N)...")
