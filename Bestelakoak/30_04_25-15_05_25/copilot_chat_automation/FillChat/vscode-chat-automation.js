...
async function automateVSCodeCopilotChat(textToSend) {
    ...
    if (folderOption) {
      ...
      if (filePickerInput) {
        ...
        if (isDOMElement(filePickerInput)) {
          ...
          filePickerInput.dispatchEvent(new InputEvent('input', { bubbles: true }));
          await wait(1000); // Wait longer after typing for suggestions to appear
          
          console.log("Typing completed, pressing Enter key and waiting for confirmation...");
          
          // Try our enhanced enter key handler with folder name
          await pressEnterAfterInput(filePickerInput, folderName);
          
          // Use our safe submit helper as a backup
          await safeSubmit(filePickerInput);
          
          // Important: Wait longer to ensure the context is fully added
          await wait(4000); // Extended wait time
          
          // Single verification block for context attachment
          ...
          const contextSelectors = [
            "#workbench\\.panel\\.chat > div > div > div.monaco-scrollable-element > div.split-view-container > div > div > div.pane-body > div.interactive-session > div.interactive-input-part > div.interactive-input-and-side-toolbar > div > div.chat-attachments-container > div.chat-attached-context",
          ...
          let contextAttachment = null;
          for (const selector of contextSelectors) {
            contextAttachment = document.querySelector(selector);
            if (contextAttachment) {
              console.log(`Found context using selector: ${selector}`);
              break;
            }
          }
          ...
          if (!contextAttachment) {
            try {
              contextAttachment = await waitForElement('.chat-attached-context', 5000)
                .catch(() => {
                  return waitForElement('.interactive-session .attachment', 5000)
                    .catch(() => null);
                });
            } catch (e) {
          ...
          if (contextAttachment) {
            ...
            if (attachmentText.toLowerCase().includes('cypress-realworld-app')) {
              ...
              // If there's any attachment, we assume it worked
              if (attachmentText.trim() !== '') {
                console.log("Context has some content, proceeding with chat");
              } else {
                console.warn("Context attachment appears to be empty");
              }
            }
          } else {
            ...
            // Alert the user to fix context manually before proceeding
            console.log("%c ⚠️ Please manually verify context is added before continuing! ⚠️", 
  ...
  await wait(2000); // Extra safety delay between context addition and input
