...
import { SimplifiedCopilotChat } from './simplifed-chat';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const sendMessageCommand = vscode.commands.registerCommand(
        'copilot-chat-automator.sendMessage', 
        async () => {
            ...
            if (message) {
                await SimplifiedCopilotChat.sendMessage(message);
            }
        }
    );
    
    // Register command to run an automated session
    const startAutomatedSessionCommand = vscode.commands.registerCommand(
        'copilot-chat-automator.startAutomatedSession',
        async () => {
            ...
            if (selectedPromptSet && selectedPromptSet.prompts && selectedPromptSet.prompts.length > 0) {
                ...
                await SimplifiedCopilotChat.runPromptSequence(selectedPromptSet.prompts);
            } else {
    ...
    const loadPromptSetCommand = vscode.commands.registerCommand(
        'copilot-chat-automator.loadPromptSet',
        async () => {
            ...
            try {
                ...
                // Run the prompts
                await SimplifiedCopilotChat.runPromptSequence(promptSet.prompts);
    ...
    context.subscriptions.push(
        sendMessageCommand,
        startAutomatedSessionCommand,
        loadPromptSetCommand
    );
}
 ...
 */
async function findPromptSetsInWorkspace(): Promise<any[]> {
