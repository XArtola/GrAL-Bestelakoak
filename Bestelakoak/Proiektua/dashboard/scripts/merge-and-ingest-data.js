...
const LLM_NAME_MAPPING = {
  ...
  'claude_3_7_thinking': 'claude_3_7_thinking',
...
// Helper function to find test metrics with fuzzy matching for the new simplified structure
...
async function mergeDataForLLM(llmName, resultsData, metricsData) {
  ...
  if (metricsData && metricsData.testFiles && mergedData.results && mergedData.results.tests) {
    ...
    mergedData.results.tests = mergedData.results.tests.map((test, index) => {
      // Extract filename from test filePath
      const filename = test.filePath ? test.filePath.replace(/\\/g, '/').split('/').pop() : '';
      const testName = test.name || '';
      ...
      if (!filename || !testName) {
        ...
        // Clean test name for dynamic executions - handle multiple execution patterns
      const baseTestName = testName
        .replace(/\s*\(\d+\)$/, '')              // Remove (1), (2), etc.
        .replace(/\s*-\s*run\s*\d+$/i, '')       // Remove "- run 1", "- run 2", etc.
        .replace(/\s*#\d+$/, '')                 // Remove "#1", "#2", etc.
        ...
        .replace(/\s*execution\s*\d+$/i, '')     // Remove "execution 1", "execution 2", etc.
        .trim();      
      // Find matching metrics using fuzzy matching
      let testMetrics = null;
      
      // Try with original test name first
      testMetrics = findTestMetrics(metricsData.testFiles, filename, testName);
      
      // If not found, try with base test name (cleaned)
      if (!testMetrics && baseTestName !== testName) {
        testMetrics = findTestMetrics(metricsData.testFiles, filename, baseTestName);
      }
      
      // If still not found, try additional cleaning strategies
      if (!testMetrics) {
        // Try removing even more patterns that might indicate multiple executions
        const extraCleanedName = testName
          .replace(/\s*\(.*?\)$/, '')          // Remove anything in parentheses at the end
          .replace(/\s*\[.*?\]$/, '')          // Remove anything in brackets at the end
          .replace(/\s*\{.*?\}$/, '')          // Remove anything in braces at the end
          .replace(/\s*-\s*\d+$/, '')          // Remove "- 1", "- 2", etc.
        ...
        if (extraCleanedName !== testName && extraCleanedName !== baseTestName) {
          testMetrics = findTestMetrics(metricsData.testFiles, filename, extraCleanedName);
        }
      }        if (testMetrics) {
        enhancedCount++;
        ...
        const commandsArray = [];
        if (testMetrics.commands && typeof testMetrics.commands === 'object') {
          for (const [command, count] of Object.entries(testMetrics.commands)) {
        ...
        return {
          ...test,
          actionableCommands: testMetrics.actionableCommands || 0,
          commands: commandsArray,
          commandCounts: testMetrics.commands || {}, // Also store the original count object
          metricsFound: true // Flag to indicate that metrics were found for this test
        };      } else {
        // Log the test that didn't find a match
        notFoundTests.push({
          filename: filename,
          testName: testName,
        ...
        return {
          ...
          commands: [],
          commandCounts: {},
          metricsFound: false // Flag to indicate that no metrics were found for this test
        };
      }    });
    
    // Log tests that didn't find matches
    if (notFoundTests.length > 0) {
      console.log(`\n⚠️ ${llmName}: ${notFoundTests.length} tests without metrics found:`);
      notFoundTests.slice(0, 10).forEach(test => { // Show first 10
        console.log(`   📁 ${test.filename} - "${test.testName}"`);
