...
Crawl and Save Interactions
    ...
    FOR    ${link}    IN    @{links}
        ${interaction}    Create Dictionary    Link=${link.text}    Element=Link    Result=${link.location}
        Append To List    ${interactions}    ${interaction}
        Capture Page Screenshot    ${link.text}_screenshot.png
    END
    FOR    ${button}    IN    @{buttons}
        ${interaction}    Create Dictionary    Link=${button.text}    Element=Button    Result=${button.location}
        Append To List    ${interactions}    ${interaction}
        Capture Page Screenshot    ${button.text}_screenshot.png
    END
    FOR    ${form}    IN    @{forms}
        ${interaction}    Create Dictionary    Link=${form.text}    Element=Form    Result=${form.location}
        Append To List    ${interactions}    ${interaction}
